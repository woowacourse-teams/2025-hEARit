...
import com.onair.hearit.core.domain.exception.CategoryDomainException;
...
public class Category {
    ...
    private void validateName(String name) {
        if (name == null || name.trim().isBlank() || name.length() > CATEGORY_NAME_MAX_LENGTH) {
            throw new CategoryDomainException("카테고리 이름은 15자 이하여야 합니다.");
        }
    }

    private void validateColorCode(String colorCode) {
        if (colorCode == null || !COLOR_CODE_PATTERN.matcher(colorCode).matches()) {
            throw new CategoryDomainException("컬러 코드는 '#'으로 시작하고 6자리 16진수여야 합니다.");
        }
    }
