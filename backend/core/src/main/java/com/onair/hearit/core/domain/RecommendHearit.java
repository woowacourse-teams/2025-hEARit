...
import com.onair.hearit.core.domain.exception.RecommendHearitDomainException;
...
public class RecommendHearit {
    ...
    private void validateHearit(Hearit hearit) {
        if (hearit == null) {
            throw new RecommendHearitDomainException("hearit은 null일 수 없습니다.");
        }
    }

    private void validateRecommendDate(LocalDate recommendDate) {
        if (recommendDate == null) {
            throw new RecommendHearitDomainException("recommendDate는 null일 수 없습니다.");
        }
    }
