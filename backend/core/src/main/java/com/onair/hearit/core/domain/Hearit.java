...
public class Hearit {
    ...
    private void validateCategory(Category category) {
        if (category == null) {
            throw new HearitDomainException("카테고리는 반드시 입력해야합니다.");
        }
    }

    private void validatePlayTime(Integer playTime) {
        if (playTime == null || playTime < 1) {
            throw new HearitDomainException("총 길이는 1초 이상의 숫자여야합니다.");
        }
    }

    private void validateSummary(String summary) {
        if (isEmptyString(summary) || summary.length() > 250) {
            throw new HearitDomainException("요약은 250자 이하의 문자열이어야합니다.");
        }
    }

    private void validateTitle(String title) {
        if (isEmptyString(title) || title.length() > 35) {
            throw new HearitDomainException("제목은 35자 이하의 문자열이어야합니다.");
        }
    }
    ...
    private void validateFileUrl(String fileUrl) {
        if (fileUrl == null) {
            throw new HearitDomainException("파일 url은 null이 될 수 없습니다.");
        }
    }

    private void validateFileType(FileType fileType) {
        if (fileType == null) {
            throw new HearitDomainException("파일 타입은 null이 될 수 없습니다.");
        }
    }
